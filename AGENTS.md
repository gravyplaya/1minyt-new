...
lib/decision.ts  JEV "System One" decision calls (typed noul/choice/score) via OpenRouter's alpha decisions endpoint — re-ranks retrieval (TAV-70)
...
- **Tickets:** features carry a `TAV-N` id. Current highest: TAV-70.
...
- **Decisions (TAV-70):** JEV typed judgments go via OpenRouter's `/api/alpha/decisions` (a different surface from chat/completions) — `lib/decision.ts`, same `OPENROUTER_API_KEY`, model pinned via `DECISION_MODEL` env (default `typesafe/jev-1.13`; never the moving `~typesafe/jev-latest` alias). Every consumer must degrade gracefully: unset key or failed call = fall back to the non-JEV path (e.g. cosine order), never break the feature. `askDecisions` returns null when the key is unset; `rerankByRelevance` swallows all failures and returns its input order.
...
| JEV retrieval re-ranking | TAV-70 | `decision.ts` (`askDecisions`, `rerankByRelevance`) | /search results, /chat + Deep Research retrieval, extension popup search (inherited) |
