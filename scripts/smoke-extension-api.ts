 ...
 * app. Checks auth (key gate + 401 paths + signed-out session guard) and the
 * read-only routes; the write path (ingest → summarize → queue) only runs
 * when SMOKE_VIDEO_ID is set, since it hits YouTube and spends LLM tokens.
 *
 ...
 *           SMOKE_SESSION_COOKIE=<1minyt_session cookie value> for the
...
async function main() {
  ...
  // TAV-68: data routes act as the session user — mirror the extension's
  // credentials: 'include' by attaching the cookie by hand.
  if (SESSION_COOKIE) auth.Cookie = `1minyt_session=${SESSION_COOKIE}`;

  console.log(`Extension API smoke test → ${BASE_URL}\n`);

  console.log('Auth (key gate):');
  ...
  if (!SESSION_COOKIE) {
    ...
    check('search without session → 401', search.status === 401, search);
    const ingest = await call('/api/extension/ingest', {
      method: 'POST',
      headers: auth,
      body: JSON.stringify({ videoId: 'dQw4w9WgXcQ' }),
    });
    check('ingest without session → 401', ingest.status === 401, ingest);
    console.log(
      '\nSigned-in checks skipped (set SMOKE_SESSION_COOKIE=<1minyt_session cookie> to run them).',
    );
  } else {
    console.log('Read-only (signed in):');
    const search = await call('/api/extension/search?q=hello', { headers: auth });
